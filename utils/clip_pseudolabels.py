...
def compute_logits(
    ...
    for img, aug_1, idxs, label, img_path in tqdm(dataloader):
        with torch.no_grad():
            if pre:
                logits_per_image, feature =  forward_method(img, pre)
            else :
                logits_per_image = forward_method(img, pre)
    ...
    if pre:
        ...
        return new_imgs, probs_list, logits, features
    else:
        probs_list = torch.cat(probs_list, dim=0).float()
        logits = torch.cat(logits, dim=0).float()
...
def compute_unlabled_logits(    
    ...
    if pre:
        filepaths, probs, output_logits, features = compute_logits(        
        ...
        return filepaths, probs, output_logits, features
    else:
...
class InstanceSelector(object):
    ...
    def __init__(self, 
        ...
        if N_PSEUDOSHOTS :
            self.Pools = PoolsAggregation(class_ids=class_ids, K=self.cfg.N_PSEUDOSHOTS, device=self.device, max_capacity_per_class=self.cfg.N_PSEUDOSHOTS)
        else:
            self.Pools = PoolsAggregation(class_ids=class_ids, K=self.cfg.N_PSEUDOSHOTS, device=self.device)
    ...
    def cal_pred_conf_uncs(self, logits, PL_labels, method):
        ...
        uncs = 1 - conf
        PL_labels = PL_labels.to(conf.device)
        # assign attrs:
        conf = conf * PL_labels
        uncs = uncs * PL_labels
    ...
    def _prepare_items_attrs(self, PL_labels, outputs, method='ul', max_num="all"):   
        """
    ...
    def select_topk_for_eachcls(self, PL_labels, indexs_all, output_all, 
                                K_max, 
                                N_iter=1, increase_percentage=0, multi_k=False):
        """
        ...
        labels, uncs = self._prepare_items_attrs(PL_labels, output_all)
        ...
        if multi_k:
            pool_caps = [K_max[i] for i in range(len(self.Pools))]
